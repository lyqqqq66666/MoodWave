...
- 登录页第三方登录里的微信图标已换成更接近官方风格的绿色双气泡样式，避免和品牌图形不一致。
- 首页与登录页里带有“实现说明 / Lottie 计划 / iOS 原型”口吻的内部文案已去掉，改成直接面向用户的陪伴表达。
- 宠物动态层先收敛到更自然的轻悬浮、呼吸和小光点环绕，先撤掉眨眼效果，避免当前图片资产阶段看起来过于刻意。
- 未登录首页恢复自然纵向滚动，首屏以下的内容不再被“海报式”布局截断。
- 音乐页听后感链路已补充“音乐场景专属 fallback”：
  - 确认当前外部 AI 调用失败时，后端会因为连接错误退回本地模板。
  - 现在退回时会结合当前曲目标题和听歌场景生成更贴近音乐页的陪伴语，而不是通用情绪模板。
  - 前端不再把这类结果直接展示成强报错态，整体观感更接近正常使用。
